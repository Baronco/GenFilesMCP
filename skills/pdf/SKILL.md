---
name: pdf-generation
description: Generate a PDF document from a Python script using reportlab — titles, paragraphs, tables, and images.
---

# PDF generation

Generate a `.pdf` with a Python script. Call `generate_pdf` (`python_script`, `file_name`, `images_list`). Before generating a PDF that embeds uploaded images, call `fetch_uploaded_chat_file_ids` to get their IDs.
