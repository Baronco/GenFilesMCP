...
description: Generate a Markdown (.md) document from a Python script — plain-text structured content.
---

# Markdown (.md) generation

Generate a `.md` document with a Python script. Call `generate_markdown` (`python_script`, `file_name`). Markdown output is plain text and cannot embed binary images; call `fetch_uploaded_chat_file_ids` only if the request references files already uploaded in the chat.
