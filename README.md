...
```bash
docker pull ghcr.io/baronco/genfiles-openapi:v0.4.0-alpha.8
```
...
docker run -d --restart unless-stopped -p 8016:8016 \
  ...
  ghcr.io/baronco/genfiles-openapi:v0.4.0-alpha.8
```

Or copy and paste this one-liner:

```bash
docker run -d --restart unless-stopped -p 8016:8016 -e OWUI_URL="http://host.docker.internal:3000" -e PORT=8016 -e REVIEWER_AI_ASSISTANT_NAME="GenFilesMCP" -e ENABLE_CREATE_KNOWLEDGE=false --name genfiles-openapi ghcr.io/baronco/genfiles-openapi:v0.4.0-alpha.8
```
