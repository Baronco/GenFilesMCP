...
def _derive_palette(base_hex: str, mode: str):
    ...
    # Softness band: keep enough chroma to read as colored, but well below neon so nothing
    # is glaring. Lower than before (was 0.42-0.62) per the "muy fuertes" feedback. Where a
    # surface needs legible text, callers deepen via _header_fill / _accent_on, so a softer
    # base saturation does not cost contrast.
    ...
    # light mode: medium accent + a notably LIGHTER, softer gradient end so impact slides
    # (cover/section/stat) read as a calm tinted panel instead of a strong saturated block;
    # near-white background kept barely tinted (band: bg saturation <= 0.06). Surfaces that
    # carry text auto-deepen for legibility (_header_fill / _accent_on).
...
def _theme_from_base(base_hex: str, mode: str, font: str, chart_palette: str) -> Theme:
    ...
    # Modern, clean type instead of the dated Calibri default: Segoe UI Semibold gives titles
    # presence, Segoe UI keeps body crisp. Both ship with Windows/Office, so decks render the
    # intended look on the viewer's machine. (`font` arg kept for signature compatibility.)
...
def _resolve_background(theme: "Theme", background: str):
    """Resolve a background token to (bg_rgb, txt_color). Custom slide backgrounds were retired
    for a fully cohesive deck, so every slide uses the theme's near-white background; only the
    `accent_color` token is still honored (used internally for table/header tinting), and any
    other value falls back to the theme background. Text color is auto-picked for contrast."""
...
def _resolve_slide_background(theme: "Theme", override: Optional[StyleOverride]):
    """Every slide uses the theme background for a fully cohesive deck. A custom
    `style_override.background` is still accepted in YAML (so existing decks keep parsing) but
    no longer changes the look — only `header_bar` still applies. For a stand-out "moment",
    use a cover / section_divider / stat_highlight slide instead."""
...
def _accent_on(accent_rgb: RGBColor, bg_rgb: RGBColor, target: float = 4.5) -> RGBColor:
    """Accent color safe to draw as FOREGROUND/decoration (title underline, timeline markers
    and date text, separators) on `bg_rgb`. Hue-light accents (teal/green/amber) are too pale
    at their natural lightness to read on the near-white content background, so the accent is
    deepened until it clears WCAG `target` against that background — same measured-iteration
    idea as `_header_fill`, but vs. the slide background instead of white. Hue/saturation are
    preserved; only lightness drops as needed. If it already clears the target, it's returned
    unchanged (deep accents like purple/navy keep their exact color)."""
    # Darken on a light background, but LIGHTEN on a dark one (e.g. a style_override panel) —
    # otherwise deepening a hue on a dark background only makes it less visible.
...
def add_header_bar(slide, prs, title, font_name, accent_rgb, bg_rgb=None, txt_color=None,
    ...
    # Hairline rule under the header — blended toward the title color so it stays visible on a
    # light OR a dark/custom background (never an invisible or heavy band).
...
def add_table(slide, table_def: TableData, left, top, width, height, font_name,
    ...
    _header = _header_fill(_accent)   # deepened so the white header text is legible on any hue
...
def build_cover(prs, theme: Theme, data: CoverSlide, image_registry: dict, variant: int = 0):
    ...
    # Left-aligned editorial cover: a short accent kicker, then a big title, subtitle and date,
    # vertically centered as a left-anchored block (cleaner and more designed than a centered
    # stack floating in the middle).
    KICK_H = Inches(0.085)
    TITLE_H = Inches(1.9)
...
def _build_column(slide, left, top, width, height,
    ...
    # short underline under the column title
...
def _build_timeline_horizontal(prs, theme: Theme, data: TimelineSlide):
    ...
    # Floating labels above/below the axis (no enclosing boxes) — clean and airy. The numbered
    # node + active ring still mark each step and the current milestone.
    tw = Inches(2.5)
    date_h = Inches(0.24)
    title_h = Inches(0.62)
...
def _build_timeline_vertical(prs, theme: Theme, data: TimelineSlide, image_registry: dict):
    ...
    # Continuous accent rail from first to last node.
    rail_w = Inches(0.034)
    ...
    # Cards fill their row (minus a small gap) so they never overlap and leave room for the
    # title; with more events the rows are shorter, so the title font scales down and auto-fits.
...
def build_stat_highlight(prs, theme: Theme, data: StatHighlightSlide, image_registry: dict, variant: int = 0):
    ...
    bg_rgb, txt_color = _resolve_slide_background(theme, data.style_override)
    accent = hex_to_rgb(theme.accent_color)
    accent_fg = _accent_on(accent, bg_rgb)        # legible accent for the figure on the light bg
    set_slide_background(slide, bg_rgb)
    ...
    # The figure itself is the hero — very large, in the theme accent, on the clean background.
    ...
    # Short accent rule under the figure, centered.
