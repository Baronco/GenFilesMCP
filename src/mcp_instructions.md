...
## Authoring guidance
Each tool's description is intentionally generic. The detailed, per-file-type authoring instructions live in the on-demand **skills** (one per file type: word, powerpoint, excel, pdf, markdown), loaded by the client as needed. Consult the matching skill for the script template / YAML schema / review workflow before calling a generation tool.
