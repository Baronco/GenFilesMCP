Generate an Excel (.xlsx) workbook using a **Python script** (`openpyxl`) — tables, formatting, formulas, and charts. Returns a markdown hyperlink for downloading the generated file.

Excel does not take a preloaded image list; call `fetch_uploaded_chat_file_ids` only if the request references files already uploaded in the chat.
